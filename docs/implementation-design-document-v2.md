...
4. **Session（50,000円/1回）**: 1on1 Decision Session（Upgrade申請経由のみ）

### 未提供のプロダクト構成（Parked）

5. **友人紹介LP**: 友人経由でPassを紹介するための専用LP（実装一部あり、本番では無効化）

**方針**: 計測の純度と運用品質を守るため、友人紹介LP導線は当面ローンチしません。公開LP→Fit Gate→Ready→Pass→Onboarding→Appの一本線を安定させることを最優先します。友人導線は「実装は残してよいが、提供はしない」を明確化しています。
...
| `inviteTokens` | Pass友人紹介トークン（Parked、本番では無効化） |
...
| `invitationToken` | VARCHAR(64) | 招待トークン（Parked、本番では無視） |
...
```typescript
{
  ...
  invitationToken?: string; // Parked、本番では無視
}
```
...
| Umami Dashboard Setup v2 | `docs/umami-dashboard-setup-v2.md` |
