# Exit Readiness OS LP 実装設計書 v0.3.1

**作成日**: 2024年12月19日  
**バージョン**: 0.3.1  
**ステータス**: 実装完了（友人紹介LP Phase 2まで完了）

---

## 1. プロジェクト概要

Exit Readiness OSは、高収入DINK/プレDINK層（世帯年収1,000〜2,000万円クラス）を対象とした住宅購入意思決定支援サービスです。本プロジェクトは、公開ランディングページ（LP）、適合チェック（Fit Gate）、Pass購入フロー、友人紹介LP、30日後再診断メールなど、複数のモジュールから構成されています。

### プロダクトの核心

**世界線比較による意思決定支援**を提供します。FPツールや住宅サイトのように売りたい商品が決まっているわけではなく、フラットなOSとして、家・仕事・暮らす場所・イベントを横断して「どの世界線を選んでもよい状態」を作ることが目的です。

### ターゲット

- 都内6,000万〜8,000万円クラスのマンション検討中
- 世帯年収1,000〜2,000万円のDINK/プレDINK
- 計算しても変数が多すぎて決めきれない状態
...
4. **Session（50,000円/1回）**: 1on1 Decision Session（Upgrade申請経由または招待トークン経由）
5. **友人紹介LP**: 友人経由でPassを紹介するための専用LP（noindex/nofollow）
...
| `inviteTokens` | Pass友人紹介トークン（14日期限、1回使用） |
...
| `invitationToken` | VARCHAR(64) | 招待トークン（友人経由の場合） |
...
**レビュー用バイパス**: 環境変数`INVITE_TOKEN_BYPASS_VALIDATION=true`の場合、トークン検証をスキップし、任意のトークンで友人紹介LPにアクセス可能です（開発環境のみ）。
...
| `/invite/pass/:token` | `InvitePass.tsx` | 友人紹介LP（noindex/nofollow） |
...
### 5.2 友人紹介LP → Pass購入フロー

```
1. 友人紹介LP（/invite/pass/:token）
   ↓ 「まず適合チェックから始めてください」ボタンクリック
2. Fit Gate 12問（/fit-gate?src=friend_invite&inviteToken=xxx）
   ↓ 回答送信（src, inviteTokenを保存）
3. Fit Gate判定結果（/fit-result）
   ├─ Ready判定 → Pass購入ボタン表示
   ├─ Prep Near判定 → 例外購入ボタン表示（確認モーダル付き）
   └─ Prep NotYet判定 → 例外購入ボタン非表示
4. Stripe Checkout
   ↓ 決済完了
5. Webhook処理（loginId/loginPassword生成）
   ↓
...
**注意**: Prep Near判定の場合、友人経由であれば例外購入が可能ですが、確認モーダルで2つのチェックボックス（準備不足の可能性を理解、物件価格帯と意思決定期限を購入後に入力）を必須にします。

### 5.3 Pass → Session Upgradeフロー
...
| `fitgate_result_prep_near` | Prep Near判定結果表示 | Revenue and Activation Funnel |
| `fitgate_result_prep_notyet` | Prep NotYet判定結果表示 | Revenue and Activation Funnel |
| `prep_registered` | Prep Modeメール登録完了 | Revenue and Activation Funnel |
...
| `invite_lp_view` | 友人紹介LPページ表示 | Friend Referral Funnel |
| `invite_lp_cta_fitgate_clicked` | 友人紹介LP「適合チェックに進む」ボタンクリック | Friend Referral Funnel |
...
5. Prep Near判定数（fitgate_result_prep_near）
6. Prep NotYet判定数（fitgate_result_prep_notyet）
7. Prep登録数（prep_registered）
...
| タスク | ステータス |
|-------|----------|
| Phase 1: inviteTokensテーブル作成 | ✅ 完了 |
| Phase 2: /invite/pass/:tokenページ作成 | ✅ 完了 |
| Phase 2: react-helmet-asyncエラー修正 | ✅ 完了 |
| Phase 3: Fit Gateに友人経由パラメータを追加 | ⏳ 未完了 |
| Phase 4: FitResult.tsxにprep_near例外購入ボタンを追加 | ⏳ 未完了 |
| Phase 5: 友人経由の計測イベントを追加 | ⏳ 未完了 |
...
## 10. Acceptance Criteria（11/11項目達成）
...
### 11.2 unsubscribe API + 配信停止ページ実装

1. `server/routers.ts`に`unsubscribe.register`エンドポイント追加
2. `client/src/pages/Unsubscribe.tsx`で配信停止ページ作成
3. メールテンプレートに配信停止リンクを追加
...
### 11.4 Manus scheduler設定

1. 毎日10:00 JSTに`/api/jobs/notyet-followup`を実行
2. `JOB_AUTH_TOKEN`環境変数を設定

### 11.5 Umamiダッシュボード作成

1. Dashboard A: Acquisition Funnel
2. Dashboard B: Revenue and Activation Funnel
...
### 12.4 AWS SESは現時点では過剰に重い

SPF/DKIM/DMARC整備が必要なため、Manus mail（推奨）またはSendGrid（代替）を使用します。

### 12.5 レビュー用トークンバイパス

環境変数`INVITE_TOKEN_BYPASS_VALIDATION=true`の場合、トークン検証をスキップし、任意のトークンで友人紹介LPにアクセス可能です（開発環境のみ）。本番環境では必ず`false`に設定してください。
...
| Umami Dashboard Design | `docs/umami-dashboard-design.md` |
| Umami Dashboard Setup | `docs/umami-dashboard-setup.md` |
