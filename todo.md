...
- **問題発覚**: 現在の実装では、価格正当化セクションがEvidence Packの直後（FAQの前）に配置されているが、ユーザーが具体的なシナリオ例を見る前に価格を見てしまう

### 実装内容
- [x] `client/src/pages/Home.tsx`の価格正当化セクション（S5）をEvidence Packセクションの後（FAQの直前）に移動
- [x] セクションの順序を調整：S1（Hero）→ S2（課題提起）→ S3（３つの柱）→ S4（具体的なシナリオ例）→ S4-2（Evidence Pack）→ **S5（価格正当化）** → S6（FAQ）→ S7（いま実際にできること）
- [x] Ready結果ページの価格表示は現状維持（29,800円を明示）

### 受け入れ条件
- [x] LPの最初のヒーローセクションとシナリオ例の間に価格が表示されていない
- [x] Evidence Pack後（FAQの直前）に価格正当化セクションが表示されている
- [x] Ready結果ページに価格が表示されている（現状維持）
- [x] セクションの順序が意図通りになっている

## PRICE-REMOVAL-01：LPから価格表示を完全に削除

### 背景
- インスタグラムから来たユーザーは、まだYOHACKの価値を理解していない
- LPで価格を見せると、価値を理解する前に離脱する可能性が高い
- 価格は、適合チェック完了後のReady結果ページでのみ表示すべき

### 実装内容
- [x] `client/src/pages/Home.tsx`から価格正当化セクション（662-690行目）を完全に削除
- [x] Ready結果ページ（`client/src/pages/FitResult.tsx`）の価格表示は現状維持

### 受け入れ条件
- [x] LPに価格（29,800円）が一切表示されていない（ブラウザで確認済み）
- [x] Ready結果ページには価格が表示されている（現状維持）
- [x] LPのセクション順序が適切に保たれている
